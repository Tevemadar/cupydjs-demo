...
function cubeloaded(event,worker,type){
    ...
    switch(type){
        ...
        case "rgb":
            ...
            break;
        case "idx":
            rawdata=new Uint8Array(2*64*64*64);
            for(var i=0;i<262144;i++){
                rawdata[i*2]=imgdata[i*4];
                rawdata[i*2+1]=imgdata[i*4+1];
            }
            break;
    }
    try{
        worker.postMessage({opcode:"imgloaded",data:rawdata.buffer},[rawdata.buffer]);
    } catch (ex) {
        worker.postMessage({opcode:"imgloaded",data:rawdata.buffer}); // IE comp
    }
}
